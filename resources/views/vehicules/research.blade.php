...
<?php
    ...
    ?>
    <br>
    <a href="{{route('Allvehicules')}}">Retourner a la page d'accueil </a> 
    <?php
}
else
{
...
<table>        
    <tr> <td> Id <td> Marque <td> Modele <td> Annee de Fabrication <td> Couleur <td> Moteur<td>Kilomettrage<td> Nombre de places <td>Climatisation  <td> GPS  <td> Prix <td> Image </tr>
        ...
        { 
            echo "<tr> <td>$v->id</td> <td>$v->marque</td> <td>$v->modele</td> <td>$v->anneefab</td> <td>$v->couleur</td> <td>$v->moteur</td> <td>$v->kilometrage </td><td>  $v->nbrplaces <td>$v->climatisation</td><td>$v->gps</td><td>$v->prix</td><td>$v->image</td></tr>";
        }
        ?>
        </table>
        <br>
